...
def get_energy(theta, backend, hamiltonian, device=None, noise_model=None, shots=10000, num_cnot_pairs=0, num_folding=0):
    """
    ...
    ## Check which type of encoding to use for variational ansatz
...
def compute_energy(theta, backend, hamiltonian, device=None, noise_model=None, shots=10000, num_cnot_pairs=0, num_folding=0, zero_noise_extrapolation=False):
    ...
    if zero_noise_extrapolation, compute energies using extrapolation method, otherwise just do standard energy calculation
    ...
    if( zero_noise_extrapolation and backend.name() == 'qasm_simulator' ):
        ...
        return get_energy(theta, backend, hamiltonian, device, noise_model, shots, num_cnot_pairs, num_folding)
...
def compute_energy_extrap(theta, backend, hamiltonian, device, noise_model, shots, num_cnot_pairs, num_folding):
    ...
    for num in range(iter_max+1):
        ...
        energy=get_energy(theta, backend, hamiltonian, device, noise_model, shots, num_cnot_pairs, num_folding)
...
def do_experiment(theta,hamiltonian,parameters):
    ...
    """
    ## Check for valid Hamiltonian type
    # if type(hamiltonian) == JordanWignerHamiltonian:
    #     encoding="JordanWigner"
    # elif type(hamiltonian) == GrayCodeHamiltonian:
    #     encoding="Graycode"
    ...
    number_circuit_folding = parameters['number_circuit_folding']
    zero_noise_extrapolation = parameters['zero_noise_extrapolation']
