...
def qite_output_filename(parameters):
    ...
    output_filename=f"qite_results_Nmax{Nmax:02d}_{interaction}_{encoding_label}_{backend_label}"
    if 
    if parameters["merge_step"]!=None:
        output_filename+=f"_ms{parameters["merge_step"]}"
    if parameters["number_cnot_pairs"]>0:
        output_filename+=f"_cnot{parameters["number_cnot_pairs"]}"
    if parameters["number_circuit_folds"]>0:
        output_filename+=f"_folds{parameters["number_circuit_folds"]}"
    if parameters["zero_noise_extrapolation"]:
        output_filename+="_extrapolated"
    return output_filename

def save_results(parameters,results):
...
if __name__ == "__main__":
    ...
    save_results(parameters,results)
